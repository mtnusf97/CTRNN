...
__all__ = ['ReadySetGoCTRNNRunner']


class ReadySetGoCTRNNRunner(object):
    ...
    def train(self):
        ...
        for epoch in range(resume_epoch, self.train_conf.max_epoch):
            ...
            # Training
            for iteration, (data, target, first_cue_idx, second_cue_idx, sine_last_idx) in enumerate(train_loader):
                total_iteration += 1
                optimizer.zero_grad()
                ...
                outputs, activations, hidden_states = model(data.to(self.device),
                                                            ...
                                                            outputs)
                loss = criterion(outputs, target.to(self.device), first_cue_idx, second_cue_idx, sine_last_idx,
                                 self.config)
                loss.backward()
                optimizer.step()
            ...
            val_loss = 0
            model.eval()
            for data, target, first_cue_idx, second_cue_idx, sine_last_idx in val_loader:
                activations, hidden_states, outputs = model.init_activations_outputs(batch_size=self.train_conf.
                                                                                     batch_size)
                outputs, activations, hidden_states = model(data.to(self.device),
                                                            activations.to(self.device),
    ...
    def test(self):
        ...
        total_test_loss = 0.0
        model.eval()
        for data, target, first_cue_idx, second_cue_idx, sine_last_idx in test_loader:
            activations, hidden_states, outputs = model.init_activations_outputs(batch_size=self.train_conf.
                                                                                 batch_size)
            outputs, activations, hidden_states = model(data.to(self.device),
                                                        activations.to(self.device),
                                                        hidden_states.to(self.device),
                                                        outputs)
            loss = criterion(outputs, target.to(self.device), first_cue_idx, second_cue_idx, sine_last_idx, self.config)
