...
__all__ = ['BeatsChainCTRNNRunner']


class BeatsChainCTRNNRunner(object):
    ...
    def __init__(self, config):
        ...
        self.config.save_dir = config.save_dir
    ...
    def train(self):
        ...
        # setting data loader
        self.config.dataset.phase = 'train'
        ...
        for epoch in range(resume_epoch, self.train_conf.max_epoch):
            ...
            # Training
            for iteration, (data, target, first_beat_idx, data_y_start_idx, data_y_end_idx) in enumerate(train_loader):
                total_iteration += 1
                optimizer.zero_grad()
                ...
                outputs, activations, hidden_states = model(data.to(self.device),
                                                            ...
                                                            outputs)
                loss = criterion(outputs, target.to(self.device), first_beat_idx, data_y_start_idx, data_y_end_idx,
                                 self.config)
                loss.backward()
                optimizer.step()
            ...
            val_loss = 0
            model.eval()
            for data, target, first_beat_idx, data_y_start_idx, data_y_end_idx in val_loader:
                activations, hidden_states, outputs = model.init_activations_outputs(batch_size=self.train_conf.
                                                                                     batch_size)
                outputs, activations, hidden_states = model(data.to(self.device),
                                                            activations.to(self.device),
            ...
            if (epoch + 1) % self.train_conf.snapshot_epoch == 0:
                if val_loss < minimum_val_loss:
                    ...
                    logger.info("No better result @ epoch {:04d}, model isn't saved!".format(epoch + 1))

            # Printing epoch results
            model.train(is_training)
            avg_train_loss = train_loss / len(train_index)
            logger.info("Avg training Loss @ the end of epoch {:04d} = {}".format(epoch + 1, avg_train_loss))
    ...
    def test(self):
        ...
        # setting data loader
        self.config.dataset.phase = 'test'
        ...
        total_test_loss = 0.0
        model.eval()
        for data, target, first_beat_idx, data_y_start_idx, data_y_end_idx in test_loader:
            activations, hidden_states, outputs = model.init_activations_outputs(batch_size=self.train_conf.
                                                                                 batch_size)
            outputs, activations, hidden_states = model(data.to(self.device),
                                                        activations.to(self.device),
                                                        hidden_states.to(self.device),
                                                        outputs)
            loss = criterion(outputs, target.to(self.device), first_beat_idx, data_y_start_idx, data_y_end_idx, self.config)
            total_test_loss += loss.item() * data.size()[0]
        total_test_loss /= len(test_dataset)
        logger.info("test Loss = {}".format(total_test_loss))
        self.writer.add_text(self.dataset_conf.name + '_test_loss',
                             f"test loss on {self.config.dataset.name} is {total_test_loss}")
        self.writer.close()
